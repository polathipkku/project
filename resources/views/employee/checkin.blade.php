...
<body>
            ...
            <div class="px-4 sm:px-6 flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                ...
                <div class="w-full sm:w-auto">
                    <input id="checkin-date" type="text"
                        ...
                        placeholder="เลือกวันที่เช็คอิน" value="{{ $displayDate }}" />
                </div>
            </div>
            ...
            <div class="overflow-x-auto mx-5">
                <table class="w-full border-collapse bg-white rounded-lg overflow-hidden shadow-md ">
                    ...
                    <tbody id="booking-rows">
                        ...
                        <tr class="border-b border-gray-200 hover:bg-gray-50 transition duration-150 booking-row"
                            data-checkin-date="{{ \Carbon\Carbon::parse($details[0]->checkin_date)->format('Y-m-d') }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            flatpickr("#checkin-date", {
                dateFormat: "d/m/Y", // รูปแบบแสดงผลเป็น d/m/Y
                locale: "th",
                defaultDate: "today",
                onChange: function(selectedDates, dateStr, instance) {
                    // แปลงรูปแบบวันที่เป็น Y-m-d สำหรับใช้เปรียบเทียบ
                    const formattedDate = formatDateForComparison(selectedDates[0]);
                    filterBookings(formattedDate);

                    // อัปเดต URL หรือส่งค่าไปยัง Controller ถ้าต้องการ
                    updateUrlWithDate(formattedDate);
                }
            });

            // ฟังก์ชันช่วยแปลงวันที่เป็นรูปแบบ Y-m-d สำหรับการเปรียบเทียบ
            ...
            // ฟังก์ชันอัปเดต URL หรือส่งค่าไปยัง Controller
            function updateUrlWithDate(dateStr) {
                // เพิ่มค่าวันที่เป็นพารามิเตอร์ใน URL
                const url = new URL(window.location.href);
                url.searchParams.set('date', dateStr);
                window.history.pushState({}, '', url.toString());

                // หรือถ้าต้องการโหลดข้อมูลใหม่จาก Controller
                // window.location.href = `/employee/checkin?date=${dateStr}`;
            }

            function filterBookings(dateStr) {
                console.log("กำลังกรองข้อมูลสำหรับวันที่:", dateStr); // Debug

                const rows = document.querySelectorAll('.booking-row');
                let visibleRows = 0;

                rows.forEach(row => {
                    const rowDate = row.dataset.checkinDate;
                ...
                console.log("พบแถวที่ตรงกัน:", visibleRows); // Debug
            ...
            // กรองข้อมูลเริ่มต้น โดยใช้วันที่จาก URL parameter หรือวันที่ปัจจุบัน
            const urlParams = new URLSearchParams(window.location.search);
            const dateParam = urlParams.get('date');

            if (dateParam) {
                // ถ้ามี parameter date ใน URL ให้ใช้ค่านั้น
                filterBookings(dateParam);

                // อัปเดต flatpickr ให้แสดงวันที่ตาม parameter
                const dateParts = dateParam.split('-');
                const selectedDate = new Date(dateParts[0], dateParts[1] - 1, dateParts[2]);
                const flatpickrInstance = document.querySelector("#checkin-date")._flatpickr;
                flatpickrInstance.setDate(selectedDate);
            } else {
                // ถ้าไม่มี parameter ให้ใช้วันที่ปัจจุบัน
                const today = new Date();
                filterBookings(formatDateForComparison(today));
            }
        });
